...
$DBAid_src = "$SourceRootFolder\DBAid-build_5.1.0" # Root folder for dbaid source files.
...
$client_domain = "@client.co.nz"             # Domain name (FQDN) client machine is in
$publickey = "Generate public key using CLR project dbaid.keygen"
$EmailEnable = "false"                       # Collector to email files? true/false
$EmailSmtp = "mailhost.client.co.nz"         # Mail relay host for sending emails
$EmailTo = "repository@client.co.nz"         # Who to send collector files to
$EmailFrom = "server@client.co.nz"           # Who the collector files came from (usually <hostname|VNN@domain.co.nz)
...
# if this is upgrade stuff, use the upgrade script!
try {     
  ...
  Write-Host "Some sort of terminating error deploying collector" -ForegroundColor Red
  $error
  Exit    
}
...
try {
  ...
  Write-Host "Some sort of terminating error deploying Config Genie"
  $error
  Exit    
}
...
try {
  ...
  if ($deploy_checkmk -eq 1) {
    if ($SQLInstance -eq "MSSQLSERVER") {
      $connectionstring = "Server=$hostname;Database=$dbaid_db_name;Trusted_Connection=True;Application Name=Check_MK;"
    }
    else {
      $connectionstring = "Server=$hostname\$SQLInstance;Database=$dbaid_db_name;Trusted_Connection=True;Application Name=Check_MK;"
    }
  ...
  Write-Host "Some sort of terminating error deploying Check_MK plugin" -ForegroundColor Red
  $error
  Exit    
}
